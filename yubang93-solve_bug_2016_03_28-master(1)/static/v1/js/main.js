mui('.mui-bar-tab').on('tap', 'a', function() {
        ...
        function show_or_hide_search_div(){
         ...
         mui.ajax('/api/v1/getRecommendedForYou/1/5',{
            ...
            success:function(data){
                ...
                for(var i in data['objs']){
                    if(data['objs'][i]['redundancy_labels']==''){
                        data['objs'][i]['resources_key'] = []
                    }else{
                        data['objs'][i]['resources_key'] = data['objs'][i]['redundancy_labels'].split("#")
                    }
                }
                ...
                new Vue({
                    ...
                    data: {"objs": data['objs']},
                    compiled:function(){
                        $('a[own-d1="wn"]').attr("style", "display:block;")
                        $("#loading_div1").hide()
                    }
                })
        ...
        mui.ajax('/api/v1/getEditorRecommend/1/5',{
            dataType:'json',//服务器返回json格式数据
            type:'get',//HTTP请求类型
            timeout:10000,//超时时间设置为10秒；
            success:function(data){
                ...
                new Vue({
                    ...
                    data: {"objs": data['objs']},
                    compiled:function(){
                        $('a[own-d1="xb"]').attr("style", "display:block;")
                        $("#loading_div2").hide()
                    }
                })
